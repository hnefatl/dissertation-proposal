...
languge\footnote{https://eta-lang.org/docs/user-guides/eta-user-guide/introduction/what-is-eta\#motivation}. The
motivation behind this project is simply individual learning -- Haskell has a number of aspects which aren't covered in
undergraduate courses, such as type-classes and lazy evaluation, which I'm very interested in learning how to implement.
...
Simple operations like addition and subtraction on integers are provided by \haskell{Int}'s \haskell{Num} instance). I
intend to implement typeclasses\footnote{http://homepages.inf.ed.ac.uk/wadler/papers/classhask/classhask.ps}, aspects of
functions (currying, partial application, recursion), arithmetic, boolean operations, lists (and functions for
...
of the ``unusual'' aspects of Haskell that are feasible to be implemented, so are the most educational to implement.
...
\item
{
    ...
    appropriately. Lazy evaluation can be tested similarly, by evaluating programs involving infinite lists and
    \haskell{undefined}.
}
\item
{
